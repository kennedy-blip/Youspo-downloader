...
def get_youtube_info(url):
    ...
    # Path for local testing (Windows/Mac)
    local_secret = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'cookies.txt')
...
def download_youtube_media(url, format_type):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if format_type == 'mp3': filename = os.path.splitext(filename)[0] + '.mp3'
